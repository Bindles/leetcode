...
var matrixSum = function(nums) {
  const sortedRows = nums.map(row => row.sort((a, b) => a - b));
  const transposed = sortedRows[0].map((_, colIndex) => sortedRows.map(row => row[colIndex]));
  const maxOfColumns = transposed.map(column => Math.max(...column));
  return maxOfColumns.reduce((acc, val) => acc + val, 0);
}
